...
有効にすると、このオプションはコンテンツ内でH1が使用されるのを防ぎます。深度に基づいて通常H1が割り当てられる見出しも、代わりにH2に昇格されます。

**注意**: このオプションは、**H1-H3**のようにH1を含むプリセットを使用している場合にのみ効果があります。H2-H6やH2-H4のようなプリセットを使用している場合、プリセットの範囲によってH1は既に除外されているため、このオプションは追加の効果を持ちません。

これは、各ページに1つのH1（タイトル）のみを持つべきというセマンティックHTML構造を維持するのに便利です。

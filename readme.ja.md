...
1. 目次機能（自動生成）
2. 階層番号付与機能（ファイルベースモデル専用）
3. Markdown見出しレベルの自動調整（ファイルベースモデル専用）🆕
4. 左サイドバーの自動表示/非表示機能 
5. ビジュアルタイマー
...
1. **Markdown見出しレベルの自動調整** 🆕（ファイルベースモデル専用）
...
1. 階層番号付与機能（ファイルベースモデル専用）
   ...
   - クリーンアップ機能: 現在のページから見出し番号を削除
   > 注意: 安全性のため、ローカルのファイルベースモデルのみで動作します

1. Markdown見出しレベルの自動調整機能 🆕（ファイルベースモデル専用）
   - 見出しレベルの自動正規化の有効/無効

...
export const whenOpenJournals = (journalsEle: HTMLDivElement) => {
    ...
    if (element) {
        if (journalsEle) {
            ...
            element.innerHTML = ""
            getJournalTitles(journalsEle, element)
        } else
            element.innerHTML = t("No headers found") //HOMEの場合
    }
}

const getJournalTitles = (journalsEle: HTMLDivElement, tocContentEle: HTMLDivElement) => {
...
const updateJournalList = (journalsEle: HTMLDivElement, tocContentEle: HTMLDivElement) => {
    tocContentEle.innerHTML = ""
    const ulEle = document.createElement("ul")
    //list-style
    ulEle.style.listStyle = "disc"
    ulEle.style.marginLeft = "3em"
    const journalTitles = journalsEle.querySelectorAll("a.journal-title") as NodeListOf<HTMLAnchorElement>
